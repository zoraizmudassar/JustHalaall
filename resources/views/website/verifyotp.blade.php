							...
							<div class="billing-address-form">
								<form action="{{ route('verify_otp') }}" method="post" enctype="multipart/form-data">
									@csrf							        
										...
										@if($errors->has('otp'))
											<span class="badge displayBadges py-2 text-light mt-2" style="background: #cd3f3f; display: block; font-size: 13px !important;">{{$errors->first('otp')}}</span>
										@endif		
							
							
										<button type="submit" class="boxed-btn">Verify</button>
								</form>
							</div>
							</div>
						</div>
